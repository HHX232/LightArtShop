...
function pause(elt) {
  ...
  if (targetPlay.classList.contains("chart__item-btn--statusActive")) {
    document
      .querySelector(`#${targetPlayButton}`)
      .classList.remove("chart__item-btn--statusActive");
  } else {
    ...
    document
      .querySelector(`#${targetPlayButton}`)
      .classList.toggle("chart__item-btn--statusActive");
  }
}
...
colorBtn.addEventListener("click", () => {
  ...
  if (body.classList.contains("body__white")) {
    ...
    colorBtn.innerText = theme;
  } else {
   let theme = "Светлая тема";
...
function activateFilter(ev) {
  ...
  const currentCheckMark = document.querySelector(
    `[data-checkIMG=${targetElID}]`
  );
